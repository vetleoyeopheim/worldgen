    ...
    for t in range(1,nt,1):
        ...
        for i in range(0,ny,1):
            ...
            for j in range(0,nx,1):
                if j == 0:
                    u = sol_x[t - 1][ny - 1][nx - 1]
                    v = sol_x[t - 1][ny - 1][nx - 1]
                    T = temp[0][i][j]
                elif j == nx - 1:
                    u = 0.0
                    v = 0.0
                    ...
                    u = sol_x[t - 1][i][j]\
                            - r1 * sol_x[t - 1][i][j] * (sol_x[t - 1][i][j] - sol_x[t - 1][i - 1][j])\
                            ...
                            + (0.1 + inv_lat[i][j]) * c * dt\


                    v = sol_y[t - 1][i][j]\
                            - r2 * sol_y[t - 1][i][j] * (sol_y[t - 1][i][j] - sol_y[t - 1][i][j - 1])\
                    ...
                    T = temp[t - 1][i][j]\
                            + dt * sol_x[t - 1][i][j] * (temp[t - 1][i][j] - temp[t - 1][i - 1][j])\
                            + dt * sol_y[t - 1][i][j] * (temp[t - 1][i][j] - temp[t - 1][i][j - 1])\
                            + dt * alpha * (temp[t - 1][i + 1][j] + temp[t - 1][i - 1][j] - 2 * temp[t - 1][i][j])\
                            + dt * alpha * (temp[t - 1][i][j + 1] + temp[t - 1][i][j - 1] - 2 * temp[t - 1][i][j])\
                            + dt * 0.2 * np.sin(lat_map[i][j] * ((0.5 * 3.14 * t) / nt)) * temp[t - 1][i][j]
